...
##                                            ##
########################333333##################
PODMAN_IMAGE="danixu86/project-zomboid-dedicated-server"
...
##                                      ##
##########################################
LATEST_IMAGES=`curl -L -s "https://registry.hub.docker.com/v2/repositories/${PODMAN_IMAGE}/tags?page_size=1024" | jq  '.results[]["name"]' | grep -iv "latest" | sort`
...
if [ ${BUILD_UNSTABLE_VERSIONS} == true ]; then
  ...
  if [ "${LATEST_IMAGE_UNSTABLE_VERSION}" == "" ] || [ $NEW_VERSION == 1 ]; then
    ...
    # Podman: --compress no existe, se usa --squash o simplemente se omite.
    # --no-cache y --build-arg funcionan igual que en Docker.
    podman build --squash --no-cache --build-arg STEAMAPPBRANCH=unstable \
      -t ${PODMAN_IMAGE}:latest-unstable \
      -t ${PODMAN_IMAGE}:${LATEST_UNSTABLE_VERSION}-unstable .
    podman push ${PODMAN_IMAGE}:${LATEST_UNSTABLE_VERSION}-unstable
    podman push ${PODMAN_IMAGE}:latest-unstable
  elif [ $NEW_VERSION == 0 ]; then
    echo -e "\n\nThere is no new unstable version of the Zomboid server\n\n"
  elif [ $NEW_VERSION == -1 ]; then
    echo -e "\n\nServer unstable version (${LATEST_UNSTABLE_VERSION}) is lower than latest image version (${LATEST_IMAGE_UNSTABLE_VERSION})... Please, check this script because maybe is not working correctly\n\n"
  else
...
if [ "${LATEST_IMAGE_STABLE_VERSION}" == "" ] || [ $NEW_VERSION == -1 ]; then
  ...
  # Podman: misma lógica que arriba. Además se corrige el bug del original:
  # usaba LATEST_SERVER_VERSION (no definida) en lugar de LATEST_STABLE_VERSION.
  podman build --squash --no-cache \
    -t ${PODMAN_IMAGE}:latest \
    -t ${PODMAN_IMAGE}:latest-release \
    -t ${PODMAN_IMAGE}:${LATEST_STABLE_VERSION}-release .
  podman push ${PODMAN_IMAGE}:${LATEST_STABLE_VERSION}-release
  podman push ${PODMAN_IMAGE}:latest-release
  podman push ${PODMAN_IMAGE}:latest
elif [ $NEW_VERSION == 0 ]; then
  echo -e "\n\nThere is no new stable version of the Zomboid server\n\n"
elif [ $NEW_VERSION == 1 ]; then
  echo -e "\n\nServer stable version (${LATEST_STABLE_VERSION}) is lower than latest image version (${LATEST_IMAGE_STABLE_VERSION})... Please, check this script because maybe is not working correctly\n\n"
else
